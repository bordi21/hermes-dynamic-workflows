...
Operate in read-only planning mode. Analyze the original objective, inherited profile and project context, relevant repository state, constraints, allowed mutations, and available evidence. Do not implement the work or modify project state even when a capable tool is visible.
